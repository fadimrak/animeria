...
function extractSeasonPart(text) {
  if (!text) return null;
  const t = text.toLowerCase();
  const mSeason = t.match(/(?:season|s)\s*(\d+)|(\d+)(?:st|nd|rd|th)\s*season/i);
  if (mSeason) return { type: "season", num: parseInt(mSeason[1] || mSeason[2]) };
  const mPart = t.match(/part\s*(\d+)|cour\s*(\d+)/i);
  if (mPart) return { type: "part", num: parseInt(mPart[1] || mPart[2]) };
  return null;
}
...
function scoreCandidate(cand, primaryEn, primaryRom, synonyms) {
  ...
  // Strict Season / Part verification to avoid wrong season playback (e.g. S4 vs S3)
  const targetSeason = extractSeasonPart(targetText);
  const candSeason = extractSeasonPart(`${cand.name} ${cand.slug} ${cand.jp}`);

  if (targetSeason && candSeason) {
    if (targetSeason.type === candSeason.type) {
      if (targetSeason.num === candSeason.num) {
        score += 1500; // Strong bonus for matching season exactly
      } else {
        score -= 5000; // Heavy penalty for mismatched season (e.g. S4 vs S3)
      }
    }
  } else if (targetSeason && !candSeason) {
    // If target specifies season but candidate does not specify it (e.g. Season 1 vs Season 4)
    if (targetSeason.num > 1) {
      score -= 2000;
    }
  }
